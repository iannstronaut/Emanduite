...
export function GeneratorPanel({ session, onGenerated }: { session: ProjectSession; onGenerated: () => Promise<unknown> }) {
  ...
  const generate = async () => { if (!preview) return; setBusy("generate"); setError(null); try { const next = unwrap(await api.generateProject(session.path, preview.targetDirectory)); setResult(next); await onGenerated(); } catch (value) { setError(message(value)); } finally { setBusy(null); } };
